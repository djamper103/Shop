...
import { AuthProvider } from "./Common/Login/AuthContext";
...
function App() {
  ...
  const removeFromFavorites = (productToRemove) => {
    setFavorites(
      favorites.filter((product) => product.id !== productToRemove.id)
    );
  };
        ...
        item.id === product.id
          ? item.count > 1
            ? {
                ...item,
                count: item.count - 1,
                priceCount: item.priceCount - item.priceCount / item.count,
              }
            : item
          : item
      )
  ...
  useEffect(() => {
    axios.get(`/api/shopItemAll`).then((response) => {
          ...
          >
            <div className="w-100" style={{ maxWidth: "400px" }}>
              <Route
                ...
                path="/Login"
                render={() => (
                  <Login cart={cart.length} setLoadingg={setLoadingg} />
                )}
              />
